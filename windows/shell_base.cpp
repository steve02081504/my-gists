...
namespace terminal_n {
	WCHAR peek_key_input() noexcept {
		constexpr size_t irBufSize = 64;
		HANDLE			 hInput	   = GetStdHandle(STD_INPUT_HANDLE);
		DWORD			 cNumRead;
		INPUT_RECORD	 irBuf[irBufSize];
		//peek next input event
		PeekConsoleInputW(hInput, irBuf, irBufSize, &cNumRead);
	...
	inline auto						  hOut = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	public:
		terminal_runner(terminal* pt) noexcept:
			base(pt) {}
			...
			floop {
				...
				floop {
					...
					if(key.wVirtualKeyCode == VK_LEFT)
						...
						else if(command.insert_index >= key.wRepeatCount)
					...
					if(key.wVirtualKeyCode == VK_RIGHT)
						...
						else if(command.insert_index + key.wRepeatCount <= command.command.size())
							move_insert_index(+(ptrdiff_t)key.wRepeatCount);
						else {
	...
	editting_command_t terminal::terminal_quick_move_left(const editting_command_t& command, size_t move_num) {
		...
		while(move_num-- && new_command.insert_index) {
			const size_t new_index = new_command.find_forward_diff_ch_type();
	...
	inline editting_command_t editting_command_t::insert(wchar_t insert_char, size_t repeat_count) const& {
		auto aret = *this;
		move(aret).insert(insert_char, repeat_count);
		return aret;
	}
	...
	inline editting_command_t editting_command_t::erase_with_no_move(size_t erase_size) const& {
		auto aret = *this;
		move(aret).erase_with_no_move(erase_size);
		return aret;
	}
	...
	inline void base_reprinter_t::operator()(const wstring& str) noexcept {
		...
		if(temp_pos.Y < end_pos.Y || (temp_pos.Y == end_pos.Y && temp_pos.X <= end_pos.X)) {
			...
			//输出空格
			out << X_times(std::max(space_num, 1u), L' ');
			move_to(temp_pos);
		}
		//更新结束位置
		end_pos = temp_pos;
	}
	inline void base_reprinter_t::move_to(COORD pos) noexcept {
