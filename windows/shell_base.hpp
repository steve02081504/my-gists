...
namespace terminal_n{
	...
	struct editting_command_t{
		wstring command;
		size_t insert_index;
		editting_command_t(const wstring& command,size_t insert_index):command(command),insert_index(insert_index){}
		editting_command_t(const wstring& command):command(command),insert_index(command.size()){}
	...
	public:
		...
		COORD get_window_size()const noexcept{
			return update_infos(),BufferInfo.dwSize;
		}
		SHORT get_buffer_width()const noexcept{
	...
	public:
		...
		void operator()(const wstring&str)noexcept{
			//简单调用需要考虑窗口大小变化的情况
			window_resize(reprinter.get_window_size(),pos_recorder.get_content().size());//简单调用总假设光标在最后
	...
	protected:
		...
		virtual wstring terminal_command_prompt(){using namespace literals;return L">> "s;}
